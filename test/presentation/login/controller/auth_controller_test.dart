...
import 'package:finance_app/presentation/login/controller/auth_state.dart';
...
void main() {
  ...
  group('sign in controller login', () {
    test('login controller success state', () async {
      expect(authController.state.value, isInstanceOf<InitialAuthState>());
      when(
      ...
      expect(authController.state.value, isInstanceOf<SuccessAuthState>());
    });
    test('login controller no user returned, throw exception', () async {
      ...
      await authController.login('user@email.com', 'user@123');
      expect(authController.state.value, isInstanceOf<ErrorAuthState>());
    });
